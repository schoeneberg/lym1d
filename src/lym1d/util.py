...
class OptionDict(dict):
  ...
  def __setitem__(self, key, val):
    if key not in self._keys:
      raise KeyError(key)
    if type(val)!=bool:
      raise ValueError("Any option can only be bool! You tried to set '{}'='{}'".format(key,val))
  ...
  # Make sure that the update routine behaves as expected, not (!) putting new options in!
  def update(self, dic):
